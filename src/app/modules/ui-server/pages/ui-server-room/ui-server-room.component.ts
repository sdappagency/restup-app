...
@Component({
  selector: 'app-ui-server-room',
  templateUrl: './ui-server-room.component.html',
  styleUrls: ['./ui-server-room.component.scss']
})
export class UiServerRoomComponent implements OnInit {
  orders: any[] = [];
  selectedProduct: any;
  ...
  getOrdersPending() {
    this.subscription = this.ordersPendingService.getOrdersPending().pipe(
      ...
      this.orders = orders.filter(order => order.type === 'en salle' && order.status !== 'en cours' && order.status !== '');
    })
  }
